"""Generate figures for history-aware Head32 binary classification."""
...
def _validate_complete(output_dir, regression=False):
...
def plot_training_curves(history, metrics):
    ...
    figure.suptitle(
        "Aligned 20-frame Head32 binary classification: training history\n"
...
def plot_test_metrics(metrics, balance):
    ...
    for axis, (metric, ylabel, baseline) in zip(axes.flat, specifications):
        ...
        axis.legend()
    figure.suptitle(
        "Aligned 20-frame Head32 binary classification: video-level test metrics",
        fontsize=15,
    )
    figure.tight_layout()
...
def main(output_dir=None, regression_dir=None):
    global OUTPUT_DIR, FIGURE_DIR, TASKS, ARCHITECTURES
    if output_dir is not None:
        OUTPUT_DIR = Path(output_dir).resolve()
    FIGURE_DIR = OUTPUT_DIR / "figures"
    _style()
    run_index = pd.read_csv(OUTPUT_DIR / "run_index.csv")
    completed = run_index.loc[run_index["status"].eq("ok")]
    available_targets = set(completed["target"].astype(str))
    available_architectures = set(completed["architecture"].astype(str))
    ...
    if not TASKS or not ARCHITECTURES:
        raise RuntimeError(f"No completed supported jobs in {OUTPUT_DIR}")
    ...
    print(f"Saved 6 classification figures and 1 table to {FIGURE_DIR}")
