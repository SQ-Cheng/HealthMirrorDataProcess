"""ImageNet backbones fused with prior-lab history for binary classification."""
...
class FusionHead(nn.Module):
    def __init__(self, image_features, dropout=0.25):
        ...
        self.classifier = nn.Sequential(
            nn.Linear(image_features + HISTORY_OUTPUT_FEATURES, HEAD_HIDDEN_FEATURES),
    ...
    def forward(self, image_features, history, history_mask):
        ...
        return self.classifier(torch.cat((image_features, history_features), dim=1))


class ImageHistoryClassifier(nn.Module):
...
def build_pretrained_model(architecture, weights_dir):
    ...
    model = ImageHistoryClassifier(backbone, image_features)
