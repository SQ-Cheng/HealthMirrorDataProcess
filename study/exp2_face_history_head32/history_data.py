...
def _standardized_distance(target, values, thresholds):
    definition = SCORE_DEFINITIONS[target]
    values = np.asarray(values, dtype=np.float64)
    thresholds = np.asarray(thresholds, dtype=np.float64)
    if definition["direction"] == "low":
        return (thresholds - values) / float(definition["scale"])
    if definition["direction"] == "high":
        return (values - thresholds) / float(definition["scale"])
    raise ValueError(f"Unsupported history direction for {target}")


def build_history_artifacts(target, records, base_manifest, output_dir):
    ...
    for row in enriched.itertuples(index=False):
        ...
        thresholds = np.full(len(values), float(row.score_threshold), dtype=np.float64)
        distances = _standardized_distance(target, values, thresholds)
        value_features = np.arcsinh(distances)
            ...
            long_rows.append(
                {
                    ...
                    "history_standardized_distance": float(distances[history_index]),
...
def write_history_manifest(summaries, output_dir):
        ...
        json.dump(
            {
                ...
                "value_feature": "asinh(clinically standardized abnormal distance)",
                "time_feature": (
