...
COMMAND="set -o pipefail; cd ${ROOT_DIR} && source /root/miniconda3/etc/profile.d/conda.sh && conda activate healthmirrorenv && MKL_THREADING_LAYER=GNU PYTHONUNBUFFERED=1 python -u -m study.exp2_face_pretrained_allframes_head32_views3.run_all --index-dir ${INDEX_DIR}${EXTRA_ARGS} 2>&1 | tee ${LOG_FILE}"
