"""Training for the controlled video-only Mamba abnormal-score ablation."""
...
from study.exp2_video_ecg_mamba.train import _regression_metrics
...
from .data import VideoOnlyWindowDataset, collate_windows
...
def _loader(windows, training, seed, persistent=True):
    ...
    return DataLoader(
        VideoOnlyWindowDataset(windows, training=training),
...
def _to_device(batch, device):
    result = dict(batch)
    for key in ("frames", "frame_times", "frame_lengths", "targets"):
...
def _plot_history(history, path, target):
    ...
    for axis in axes:
        ...
        axis.legend()
    figure.suptitle(f"Native-video-only Mamba ablation | {target}")
    figure.tight_layout()
...
def train_task(
    ...
    _log(
        f"[job-start] target={target} device={device} modality=video_only "
        ...
        f"mamba_parameters={counts['mamba']}"
    )
