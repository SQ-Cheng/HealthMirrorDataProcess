# Exp2 Binary 20 Frames: Head-32 Last-Stage, Aligned Split, Five Views

Canonical ID: `exp2_face_pretrained_head32_laststage`.

This experiment is a controlled partial-finetuning counterpart of
`exp2_face_pretrained_head64`. It uses the same corrected source records,
closest-event selection, patient-level distribution-balanced split, exact video
records, exact 20-frame index, model input, five views, seed, and two-stage
...
Stage 1 freezes the complete backbone and trains only the classification head. Stage 2
keeps all earlier backbone stages frozen and in eval mode, while training the head plus
the final semantic backbone stage: MobileNetV3-Small `features[9:13]`, or
EfficientNet-B0 `features[7:9]`. Learning rates, epoch limits, early stopping, batches,
augmentation, and evaluation are unchanged.
...
```bash
bash study/exp2_face_pretrained_head32_laststage/launch_screen.sh --overwrite
```

```bash
screen -r exp2_face_pretrained_head32_laststage
```
