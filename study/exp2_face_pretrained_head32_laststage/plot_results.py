"""Generate figures for aligned 20-frame Head32 Last-Stage binary classification."""
...
def plot_training_curves(history, metrics):
    ...
    figure.suptitle(
        "Aligned 20-frame Head32 Last-Stage binary classification: training history\n"
...
def plot_test_metrics(metrics, balance):
    ...
    for axis, (metric, ylabel, baseline) in zip(axes.flat, specifications):
        ...
        axis.legend()
    figure.suptitle(
        "Aligned 20-frame Head32 Last-Stage binary classification: video-level test metrics",
        fontsize=15,
    )
    figure.tight_layout()
