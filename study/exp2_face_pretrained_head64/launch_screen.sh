...
SESSION_NAME="${SESSION_NAME:-exp2_face_pretrained_head64}"
LOG_DIR="${ROOT_DIR}/study/exp2_face_pretrained_head64/logs"
...
COMMAND="set -o pipefail; cd ${ROOT_DIR} && source /root/miniconda3/etc/profile.d/conda.sh && conda activate healthmirrorenv && python -m study.exp2_face_pretrained_head64.prepare_inputs && MKL_THREADING_LAYER=GNU PYTHONUNBUFFERED=1 CUDA_VISIBLE_DEVICES=0,1,2,3 python -u -m study.exp2_face_pretrained_head64.run_all${EXTRA_ARGS} 2>&1 | tee ${LOG_FILE}"
