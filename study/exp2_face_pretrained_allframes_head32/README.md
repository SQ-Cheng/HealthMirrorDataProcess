# Exp2 Pretrained All Frames: Head-32
...
has the same five deterministic views as the parent experiment; validation and test
use the original view. All other model, split, optimization, and early-stopping settings
match `../exp2_face_pretrained_head32`.
...
```bash
bash study/exp2_face_pretrained_allframes_head32/launch_screen.sh
```

```bash
screen -r exp2_face_pretrained_allframes_head32
```
