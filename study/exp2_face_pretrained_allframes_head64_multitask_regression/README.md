...
Canonical ID: `exp2_regression_allframes_head64_multitask`.
...
- Head: Linear -> LayerNorm -> SiLU -> Dropout -> Linear(5), hidden width 64
...
```bash
python -m study.exp2_face_pretrained_allframes_head64_multitask_regression.run_all \
  --prepare-only
```
...
```bash
bash study/exp2_face_pretrained_allframes_head64_multitask_regression/launch_screen.sh
```
