"""Generate Head64 multi-output regression figures and controlled comparisons."""
...
HEAD32_MULTITASK_DIR = (
    EXP_DIR.parent
    / "exp2_face_pretrained_allframes_head32_multitask_regression"
    / "outputs"
)
SINGLE_OUTPUT_DIR = (
...
METHOD_COLORS = {
    "head64_multitask": "#197278",
    "head32_multitask": "#6B7280",
    "head32_single_task": "#E07A5F",
    "multi_output": "#197278",
    "single_task": "#E07A5F",
}
METHOD_LABELS = {
    "head64_multitask": "Head64 multi-output",
    "head32_multitask": "Head32 multi-output",
    "head32_single_task": "Head32 single-task",
    "multi_output": "Head64 multi-output",
    "single_task": "Head32 single-task",
}
...
def plot_training_curves(history):
    ...
    figure.suptitle(
        "Head64 multi-output abnormal-score regression: training history",
        fontsize=15,
    )
    figure.tight_layout()
...
def plot_test_metrics(metrics):
    ...
    figure.suptitle(
        "Head64 multi-output regression: video-level test performance",
        fontsize=15,
    )
    figure.tight_layout()
...
def plot_split_generalization(metrics):
    ...
    for row, architecture in enumerate(ARCHITECTURES):
        ...
        for column, (metric, ylabel) in enumerate(
            ...
            axis.grid(axis="y", alpha=0.24)
            axis.legend()
    figure.suptitle("Head64 multi-output regression: split generalization", fontsize=15)
    figure.tight_layout()
    figure.savefig(
...
def plot_predictions(metrics):
    ...
    figure.suptitle(
        "Head64 multi-output regression: video-level test predictions",
        fontsize=15,
        y=1.002,
    )
    figure.tight_layout()
...
def _reported_comparison(head64_metrics, head32_metrics, single_metrics):
    frames = []
    for method, frame in (
        ("head64_multitask", head64_metrics),
        ("head32_multitask", head32_metrics),
        ("head32_single_task", single_metrics),
    ):
        selected = frame.loc[
...
def plot_reported_comparison(comparison):
    ...
    for row, architecture in enumerate(ARCHITECTURES):
        for column, (metric, ylabel, ylim) in enumerate(specifications):
            ...
            axis.legend()
    figure.suptitle(
        "Reported test performance across three experiments\n"
        "Head64 and Head32 multi-output results are paired; "
        "single-task Head32 uses an independent patient split",
        fontsize=14,
    )
    figure.tight_layout()
    figure.savefig(
...
def plot_head_width_training(head64_history, head32_history):
    figure, axes = plt.subplots(2, 2, figsize=(15, 10), squeeze=False)
    specifications = (
        ("val_loss", "Validation task-balanced SmoothL1"),
        ("val_macro_mae", "Validation macro MAE"),
    )
    histories = (
        ("head64_multitask", head64_history),
        ("head32_multitask", head32_history),
    )
    for row, architecture in enumerate(ARCHITECTURES):
        for column, (metric, ylabel) in enumerate(specifications):
            axis = axes[row, column]
            for method, history in histories:
                ...
                finetune = selected.loc[
                    selected["stage"].eq("finetune"), "global_epoch"
                ]
                if not finetune.empty:
                    axis.axvline(
                        finetune.min() - 0.5,
            ...
            axis.legend()
    figure.suptitle(
        "Controlled head-width comparison: validation history\n"
        "Dotted lines mark each run's backbone-unfreeze boundary",
        fontsize=14,
    )
    figure.tight_layout()
    figure.savefig(
        FIGURE_DIR / "head_width_training_comparison.png",
        dpi=180,
        bbox_inches="tight",
    )
    plt.close(figure)
...
def _head_width_metrics(head64_metrics, head32_metrics):
    frames = []
    for method, frame in (
        ("head64_multitask", head64_metrics),
        ("head32_multitask", head32_metrics),
    ...
    comparison = pd.concat(frames, ignore_index=True)
    comparison.to_csv(
        FIGURE_DIR / "head_width_test_metrics.csv",
        index=False,
    )
    return comparison


def plot_head_width_metrics(comparison):
    x = np.arange(len(TASKS), dtype=np.float64)
    width = 0.36
    figure, axes = plt.subplots(2, 3, figsize=(18, 10), squeeze=False)
    specifications = (
    ...
    methods = ("head32_multitask", "head64_multitask")
    for row, architecture in enumerate(ARCHITECTURES):
        for column, (metric, ylabel, ylim) in enumerate(specifications):
            axis = axes[row, column]
            for method_index, method in enumerate(methods):
            ...
            axis.legend()
    figure.suptitle(
        "Controlled Head32 vs Head64 multi-output comparison\n"
        "Identical patient split, videos, views, seeds, and optimization",
        fontsize=14,
    )
    figure.tight_layout()
    figure.savefig(
        FIGURE_DIR / "head_width_test_comparison.png",
        dpi=180,
        bbox_inches="tight",
    )
    plt.close(figure)
...
def _paired_head_width_summary():
    rows = []
    rng = np.random.default_rng(20260729)
    for architecture in ARCHITECTURES:
        head64_predictions = pd.read_csv(
            OUTPUT_DIR / "runs" / architecture / "video_predictions.csv",
            dtype={"hospital_id": str, "video_id": str},
        )
        head32_predictions = pd.read_csv(
            HEAD32_MULTITASK_DIR
            / "runs"
            / architecture
        ...
        for task in TASKS:
            columns = ["hospital_id", "video_id", "y_true", "y_pred"]
            head64 = head64_predictions.loc[
                head64_predictions["split"].eq("test")
                & head64_predictions["target"].eq(task),
                columns,
            ]
            head32 = head32_predictions.loc[
                head32_predictions["split"].eq("test")
                & head32_predictions["target"].eq(task),
                columns,
            ]
            paired = head64.merge(
                head32,
                ...
                how="inner",
                validate="one_to_one",
                suffixes=("_head64", "_head32"),
            )
            if len(paired) != len(head64) or len(paired) != len(head32):
                raise ValueError(
                    f"Head-width test sets differ for {architecture}/{task}"
                )
            if not np.allclose(
                paired["y_true_head64"],
                paired["y_true_head32"],
                atol=1e-7,
            ):
                raise ValueError(
                    f"Head-width targets differ for {architecture}/{task}"
                )
            y_true = paired["y_true_head64"].to_numpy(np.float64)
            error64 = np.abs(
                paired["y_pred_head64"].to_numpy(np.float64) - y_true
            )
            error32 = np.abs(
                paired["y_pred_head32"].to_numpy(np.float64) - y_true
            )
            difference = error64 - error32
            ...
            rows.append(
                {
                    ...
                    "head64_mae": float(error64.mean()),
                    "head32_mae": float(error32.mean()),
                    "mae_difference_head64_minus_head32": float(difference.mean()),
                    "bootstrap_95ci_low": float(
                        np.quantile(bootstrap, 0.025)
                    ),
                    "bootstrap_95ci_high": float(
                        np.quantile(bootstrap, 0.975)
                    ),
                    "fraction_head64_lower_error": float(
                        np.mean(error64 < error32)
                    ),
                    "fraction_equal_error": float(
                        np.mean(np.isclose(error64, error32, atol=1e-12))
                    ),
                }
            )
    summary = pd.DataFrame(rows)
    summary.to_csv(
        FIGURE_DIR / "head_width_paired_summary.csv",
        index=False,
    )
    return summary


def plot_paired_head_width(summary):
    x = np.arange(len(TASKS), dtype=np.float64)
    figure, axes = plt.subplots(2, 2, figsize=(15, 10), squeeze=False)
    for row, architecture in enumerate(ARCHITECTURES):
        selected = (
            summary.loc[summary["architecture"].eq(architecture)]
            .set_index("target")
            .loc[list(TASKS)]
        )
        labels = [
            f"{TASK_LABELS[target]}\nn={int(selected.loc[target, 'n_paired_test_videos'])}"
            for target in TASKS
        ]
        delta = selected["mae_difference_head64_minus_head32"].to_numpy()
        lower = delta - selected["bootstrap_95ci_low"].to_numpy()
        upper = selected["bootstrap_95ci_high"].to_numpy() - delta
        axis = axes[row, 0]
        axis.errorbar(
            x,
            delta,
            ...
            markersize=7,
            capsize=5,
            color=METHOD_COLORS["head64_multitask"],
            ecolor="#4B5563",
        )
        axis.axhline(0, color="#555555", linestyle="--", linewidth=1)
        axis.set_xticks(x, labels, rotation=23, ha="right")
        axis.set_ylabel("Paired MAE difference\n(Head64 minus Head32)")
        axis.set_title(f"{ARCHITECTURE_LABELS[architecture]} | negative favors Head64")
        axis.grid(axis="y", alpha=0.24)

        axis = axes[row, 1]
        axis.bar(
            x,
            selected["fraction_head64_lower_error"],
            width=0.62,
            color=METHOD_COLORS["head64_multitask"],
        )
        ...
        axis.set_xticks(x, labels, rotation=23, ha="right")
        axis.set_ylabel("Fraction of paired test videos")
        axis.set_title(
            f"{ARCHITECTURE_LABELS[architecture]} | Head64 lower absolute error"
        )
        axis.grid(axis="y", alpha=0.24)
    figure.suptitle(
        "Paired effect of widening the multi-output head\n"
        "Error bars are video-level bootstrap 95% confidence intervals",
        fontsize=14,
    )
    figure.tight_layout()
    figure.savefig(
        FIGURE_DIR / "head_width_paired_error.png",
        dpi=180,
        bbox_inches="tight",
    )
    plt.close(figure)
...
def main(output_dir=None, head32_multitask_dir=None, single_output_dir=None):
    global OUTPUT_DIR, HEAD32_MULTITASK_DIR, SINGLE_OUTPUT_DIR, FIGURE_DIR
    if output_dir is not None:
        OUTPUT_DIR = Path(output_dir).resolve()
    if head32_multitask_dir is not None:
        HEAD32_MULTITASK_DIR = Path(head32_multitask_dir).resolve()
    if single_output_dir is not None:
        SINGLE_OUTPUT_DIR = Path(single_output_dir).resolve()
    FIGURE_DIR = OUTPUT_DIR / "figures"
    _style()
    _validate_complete(OUTPUT_DIR)
    _validate_complete(HEAD32_MULTITASK_DIR)
    _validate_complete(SINGLE_OUTPUT_DIR)
    FIGURE_DIR.mkdir(parents=True, exist_ok=True)

    history = pd.read_csv(OUTPUT_DIR / "history_all.csv")
    head32_history = pd.read_csv(HEAD32_MULTITASK_DIR / "history_all.csv")
    head64_metrics = pd.read_csv(OUTPUT_DIR / "metrics_all.csv")
    head32_metrics = pd.read_csv(HEAD32_MULTITASK_DIR / "metrics_all.csv")
    single_metrics = pd.read_csv(SINGLE_OUTPUT_DIR / "metrics_all.csv")
    plot_training_curves(history)
    plot_test_metrics(head64_metrics)
    plot_split_generalization(head64_metrics)
    plot_predictions(head64_metrics)
    plot_head_width_training(history, head32_history)
    plot_head_width_metrics(
        _head_width_metrics(head64_metrics, head32_metrics)
    )
    plot_paired_head_width(_paired_head_width_summary())
    plot_reported_comparison(
        _reported_comparison(head64_metrics, head32_metrics, single_metrics)
    )
    plot_common_test_comparison(_common_test_comparison())
    print(f"Saved 9 figures and 4 comparison tables to {FIGURE_DIR}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--output-dir", default=None)
    parser.add_argument("--head32-multitask-dir", default=None)
    parser.add_argument("--single-output-dir", default=None)
    arguments = parser.parse_args()
    main(
        arguments.output_dir,
        arguments.head32_multitask_dir,
        arguments.single_output_dir,
    )
