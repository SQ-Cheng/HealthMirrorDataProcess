# Exp2 Binary 20 Frames: Head-32, Aligned Split, Five Views

Canonical ID: `exp2_face_pretrained_head32`.

This experiment is the binary counterpart of the Head32 single-task abnormal-score
...
used. The task records and frame offsets are reused directly from
`exp2_face_pretrained_head32_regression`; runtime validation rejects any video without
exactly 20 indexed frames or any patient assigned to multiple splits. Each training
frame has five deterministic views; validation and test use the original view.
...
```bash
bash study/exp2_face_pretrained_head32/launch_screen.sh --overwrite
```
