"""Mamba-style models for single-signal masked reconstruction.

Uses Mamba-inspired blocks: LayerNorm → Linear projection → depthwise conv → SiLU gating.
...
class GatedResidual(nn.Module):
    """Gated conv residual block: split channels, tanh*sigmoid gating, 1x1 projection."""
...
class MambaLikeBlock(nn.Module):
    """Mamba-inspired block: LayerNorm → Linear(expand) → depthwise conv → SiLU gate → Linear(project)."""
    ...
    def forward(self, x):
        y = x.transpose(1, 2)              # (B, C, L) → (B, L, C)
        y = self.norm(y)
        v, g = torch.chunk(self.in_proj(y), 2, dim=-1)  # (B, L, H)
        v = v.transpose(1, 2)              # (B, H, L)
        v = self.dwconv(v)
        v = v.transpose(1, 2)              # (B, L, H)
        y = F.silu(v) * F.silu(g)
        y = self.drop(self.out_proj(y))
        return x + y.transpose(1, 2)       # (B, C, L)
...
class SingleReconMambaLight(nn.Module):
    """Light Mamba-style model: stem → 4 MambaLikeBlocks → output head."""
...
class SingleReconMambaFull(nn.Module):
    """Full Mamba-style model: deeper stem → 6 MambaLikeBlocks → refine → output head."""
...
def build_mamba_model(variant="light"):
    """Build a Mamba-style model.
