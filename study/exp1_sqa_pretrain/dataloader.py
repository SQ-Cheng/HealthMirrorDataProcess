"""Unified dataloader for Exp1-SQAPreTrain masked reconstruction.

Supports three modes:
- signal_type='ecg':   single-channel ECG reconstruction
- signal_type='rppg':  single-channel rPPG reconstruction
- signal_type='joint': dual-channel ECG+rPPG reconstruction

Quality scores are computed from autocorrelation SQI (ECG) and narrow-band SNR (rPPG),
then rank-normalized to [0,1].
"""
...
def _resolve_mirror_dirs(root_dir, data_source):
    """Resolve mirror directories. Supports:
    - A single mirror folder (contains patient_*.csv).
    - A parent folder containing mirror*_auto_cleaned[_sqi] subfolders.
    """
...
class MaskedReconDataset(Dataset):
    """Per-window masked reconstruction dataset.

    For signal_type='ecg' or 'rppg': single-channel output (1, L).
    For signal_type='joint': dual-channel output (2, L) [ECG, rPPG].

    Quality scores:
    - ECG: autocorrelation periodicity SQI.
    - rPPG: narrow-band SNR in [0.5, 5.0] Hz.
    - Joint: weighted combination of ECG SQI (0.4) and rPPG SNR (0.6), rank-normalized.
    """
    ...
    def __init__(
        ...
        for mirror_dir in mirror_dirs:
            ...
            for fpath in signal_files:
                ...
                while start + window_samples <= len(rppg):
                    ...
                    # Build sample based on signal_type
                    if self.signal_type == "joint":
                        sample = np.stack([ecg_win, rppg_win], axis=0)  # (2, L)
                        quality_value = float(0.4 * ecg_sqi + 0.6 * rppg_snr)  # raw proxy
                    elif self.signal_type == "ecg":
                        sample = ecg_win[None, :]  # (1, L)
                        quality_value = float(ecg_sqi)
                    else:  # rppg
                        sample = rppg_win[None, :]  # (1, L)
        ...
        # Rank-normalize quality scores to [0, 1]
        self.quality_score = _rank01(self.raw_quality).astype(np.float32).tolist()

        print(
            f"[Exp1 Dataset:{self.signal_type}] Loaded {len(self.samples)} windows from "
...
def build_dataloaders(
    ...
    # Fallback to sample-level split if patient-level produces empty side
    ...
    print(
        f"[Exp1 DataLoader:{signal_type}] Patient-level split - "
