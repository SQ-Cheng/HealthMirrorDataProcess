...
def _apply_reference_split(records, target, reference_records_dir):
    path = os.path.join(reference_records_dir, f"{target}.csv")
    reference = pd.read_csv(path, dtype={"hospital_id": str, "video_id": str})
    expected = reference[["hospital_id", "video_id", "split"]].copy()
    if expected["video_id"].duplicated().any():
        raise ValueError(f"Reference split has duplicate videos for {target}")
    current_ids = set(records["video_id"].astype(str))
    reference_ids = set(expected["video_id"].astype(str))
    if current_ids != reference_ids:
        raise ValueError(
            f"Canonical {target} sample set differs from reference: "
            f"missing={len(reference_ids - current_ids)} "
            f"added={len(current_ids - reference_ids)}"
        )
    result = records.drop(columns="split", errors="ignore").merge(
        expected,
    ...
    if result["split"].isna().any():
        raise ValueError(f"Reference split assignment failed for {target}")
    if result.groupby("hospital_id")["split"].nunique().gt(1).any():
        raise ValueError(f"Patient leakage in reference split for {target}")
    summary_rows, pair_rows = _distribution_audit(result, target)
    selection = {
        "target": target,
        "accepted": True,
        "algorithm": "reuse exact reference patient split",
        "reference_records": os.path.abspath(path),
        "all_distribution_pairs_pass": bool(
            all(row["passed"] for row in pair_rows)
        ),
    }
    return result.reset_index(drop=True), "", summary_rows, pair_rows, selection
    ...
    for target in targets:
        ...
        if summary["status"] != "skipped":
            if reference_records_dir is None:
                ...
                records, reason, audit_rows, pair_rows, selection = (
                    _apply_reference_split(records, target, reference_records_dir)
                )
...
class AllFramesDataset(Dataset):
    ...
    def __init__(
        ...
        self.weights_by_video = (
            self.video_records["range_weight"].to_numpy(np.float32)
            if "range_weight" in self.video_records
            else np.ones(len(self.video_records), dtype=np.float32)
        )
    ...
    def __getitem__(self, index):
        ...
        return (
            ...
            torch.tensor(self.weights_by_video[video_row], dtype=torch.float32),
        )
