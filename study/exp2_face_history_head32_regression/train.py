...
from .models import (
    ...
    freeze_encoder,
    history_encoder_parameter_count,
    parameter_counts,
    unfreeze_all,
)
...
def _loader(frame_index, records, history_store, views, architecture, shuffle):
        ...
        result.update(
            {
                ...
                "sign_roc_auc": float(
                    roc_auc_score(sign_targets, abnormal_decision_score)
                ),
                "sign_average_precision": float(
                    average_precision_score(
                        sign_targets, abnormal_decision_score
                    )
                ),
                "tn": int(tn),
                "fp": int(fp),
...
def _train_epoch(
    ...
    for batch_index, (
        ...
        if view_codes.ndim == 2:
            ...
            history_features = history_features.repeat_interleave(
                view_codes.shape[1], dim=0
            )
            history_mask = history_mask.repeat_interleave(view_codes.shape[1], dim=0)
            sample_weights = sample_weights.repeat_interleave(view_codes.shape[1])
        labels = labels.to(device, non_blocking=True).unsqueeze(1)
        history_features = history_features.to(device, non_blocking=True)
        ...
        with torch.autocast(
            ...
            predictions = model(images, history_features, history_mask)
            loss = (criterion(predictions, labels) * sample_weights).mean()
...
def _video_metrics(evaluation, dataset, split, target_scaler):
    ...
    aggregation = pd.DataFrame({
        ...
        "y_true_scaled": evaluation["labels"].astype(np.float32),
        "y_pred_scaled": evaluation["scores"],
    ...
    video_predictions["y_true_scaled"] = aggregation["y_true_scaled"].to_numpy(
        np.float32
    )
    ...
    compact_frames = {
        ...
        "y_true": frame_true_raw.astype(np.float32),
        "y_pred": frame_pred_raw.astype(np.float32),
        "y_true_scaled": evaluation["labels"].astype(np.float32),
        "y_pred_scaled": evaluation["scores"].astype(np.float32),
    }
...
def train_task(
    ...
    history_parameters = history_encoder_parameter_count(model)
    history = []
    ...
    _log(
        ...
        f"loss_weighting={'train_range' if range_weighted else 'none'} "
        f"parameters={total_parameters} "
        f"history_encoder_parameters={history_parameters}"
    )
    ...
    for split in ("train", "val", "test"):
        ...
        compact_predictions.append({
            ...
            "y_true_scaled": frames["y_true_scaled"],
            "y_pred_scaled": frames["y_pred_scaled"],
        })
    ...
    np.savez_compressed(
        ...
        y_true_scaled=np.concatenate([
            item["y_true_scaled"] for item in compact_predictions
        ]),
        y_pred_scaled=np.concatenate([
            item["y_pred_scaled"] for item in compact_predictions
        ]),
        raw_unit=np.asarray([target_scaler.unit], dtype=str),
    )
    ...
    torch.save({
        ...
        "loss_weighting": (
            "train-only fixed-width raw-value range weights"
            if range_weighted else "none"
        ),
        "range_weighting": range_weighting,
        ...
        "history_encoder_parameters": history_parameters,
        "history_input": "all strictly prior same-analyte same-admission measurements",
