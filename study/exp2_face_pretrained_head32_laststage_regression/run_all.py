...
def _validate_reference_task_records(task_records, reference_dir):
    if not reference_dir:
        return
    for target, records in task_records.items():
        reference_path = os.path.join(reference_dir, f"{target}.csv")
        if not os.path.isfile(reference_path):
            raise FileNotFoundError(
                f"Missing reference task records for {target}: {reference_path}"
            )
        ...
        try:
            ...
            raise ValueError(
                f"Task records/split differ from Head64 reference for {target}"
            ) from exc
        print(
            f"[data-match] target={target} videos={len(current)} "
            "records_and_split=exact",
            flush=True,
        )
...
def main():
    ...
    parser.add_argument("--reference-records-dir", default=None)
    ...
    _validate_reference_task_records(
        task_records,
        args.reference_records_dir,
    )
    with open(
    ...
    experiment_manifest = {
        "schema_version": 1,
        "experiment": (
            f"exp2_raw_video_{args.frame_policy}_head32_laststage_regression_balanced_split"
        ),
        ...
        "seed": args.seed,
        "reference_task_records_dir": (
            os.path.abspath(args.reference_records_dir)
            if args.reference_records_dir
            else None
        ),
        "data_fingerprints": {
        ...
        "training": {
            ...
            "stage_2": (
                "regression head plus final semantic backbone stage, "
                f"lr={FINETUNE_LEARNING_RATE:.8g}"
            ),
            "stage_2_trainable_backbone": {
                "mobilenet_v3_small": "features[9:13]",
                "efficientnet_b0": "features[7:9]",
            },
            "frozen_batchnorm_policy": (
                "all frozen backbone stages remain in eval mode; only the final "
                "stage and regression head enter train mode"
            ),
