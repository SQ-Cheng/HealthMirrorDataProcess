"""Shared visualization for single-signal masked reconstruction (Exp3 all architectures)."""
...
def parse_args(exp_name, description_suffix=""):
    desc = f"Visualize {exp_name}{description_suffix} masked reconstruction"
...
def run_visualization(
    ...
    args = parse_args(exp_name, description_suffix=title_prefix)
    ...
    model = model_builder_fn(args.variant).to(DEVICE)
    ...
    arch_label = f"{title_prefix} " if title_prefix else ""
    fig.suptitle(
        f"{exp_name} {arch_label}Masked Reconstruction ({channel_label})\n"
    ...
    if args.output is None:
        out_dir = plot_dir or os.path.join(EXP_DIR, "plots")
