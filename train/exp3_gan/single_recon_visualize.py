"""GAN visualization for single-signal masked reconstruction (Exp3 GAN split)."""
...
from exp3_common.single_recon_train import build_single_window_visible_mask
from single_recon_model import build_exp3_gan_models
...
def parse_args(exp_name):
    parser = argparse.ArgumentParser(description=f"Visualize {exp_name} GAN masked reconstruction")
...
def load_first_compatible_checkpoint(generator, checkpoint_candidates):
    errors = []
...
def run_visualization(signal_type, exp_name):
    ...
    generator, _ = build_exp3_gan_models(args.variant)
    generator = generator.to(DEVICE)
    ckpt_path, _ = load_first_compatible_checkpoint(generator, ckpt_candidates)
    generator.eval()
    ...
    masked_mae_val = np.mean(np.abs((pred_np - signal_np) * masked_region))
    ...
    for i in range(n):
        ...
        ax.plot(t, pred_np[i], label=f"{channel_label} GAN recon", color=recon_color, linewidth=1.0)
    ...
    fig.suptitle(
        f"{exp_name} GAN Masked Reconstruction ({channel_label})\n"
        f"Checkpoint={os.path.basename(ckpt_path)} | Masked MAE={masked_mae_val:.4f}",
        y=0.995,
    )
    ...
    print(f"Masked MAE={masked_mae_val:.6f}")
